console.log("testHistory.js loaded")

// Nơi mà thử nghiệm cách sử dụng history. 
...
function getRouteFromURL() {
    ...
    if (path === "/life-lessons") return {name: "life-lessons", userRole: "user"}; 
    if (path === "/teaching-words") return {name: "teaching-words", userRole: "user"};
    if (path === "/admin/home") return {name: "home", userRole: "admin"}; 
    if (path === "/admin/life-lessons") return {name: "life-lessons", userRole: "admin"}; 
    if (path === "admin/teaching-words") return {name: "teaching-words", userRole: "admin"}; 
...
// cái này sẽ nằm ở controller, sẽ xử lí phần logic chính và sử dụng ở tầng api flow và 
async function initApp() {
    // 1. lấy token và check user, nếu không có thì về lại login 
